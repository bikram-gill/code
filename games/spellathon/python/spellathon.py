...
output_file_name - output file generated by this function
'''
def parseWordListOfGivenLength( file_name_with_path, min_length_to_filter, 
                                max_length_to_filter, leading_alphabet, output_file_name):
    try:
        words = open(file_name_with_path, 'r')
    except FileNotFoundError:
        print('File not found. A list of words, with a word in each line, should exist: ' + file_name_with_path)
    ...
    #words longer than word_length_to_filter are saved in following file
    long_words = open(DATA_FILE_PATH + FOLDER_SEPARATOR + TEMP_FOLDER + FOLDER_SEPARATOR + output_file_name, 'w')
...
'''
filterOutNonAlphabets - finds words which do not contain any alphabets in the input string

input_file_name - input file containing one word per line
non_alphabets - list of alphabets which should not exist in any word in the output file
output_file_name - file generated by this function
'''
def filterOutNonAlphabets(input_file_name, non_alphabets, output_file_name):
    ...
    try:
        ...
        print('File not found. A list of words, with a word in each line, should exist: ' + 
                    DATA_FILE_PATH + FOLDER_SEPARATOR + TEMP_FOLDER + FOLDER_SEPARATOR + input_file_name)
        return
    
    #words not containing any non-alphabets
    output_words = open(DATA_FILE_PATH + FOLDER_SEPARATOR + TEMP_FOLDER + FOLDER_SEPARATOR + output_file_name, 'w')
...
def findWordsForSpellathonSimpleSearch(alphabetList):
    ...
    2. Find all combinations of letters which are of length 4 to 7, or
    find all words which do not contain any other letters
    ...
    parseWordListOfGivenLength( DATA_FILE_PATH + FOLDER_SEPARATOR + DATA_FILE_WORDS, 
                                ...
                                alphabetList[0], DATA_FILE_LONGWORDS)

    #Find all alphabets not in spellathon query, to be used later in regex
    nonAlphabets = re.sub(r'['+ alphabetList +']','', string.ascii_lowercase)
    
    #Find all words which do not contain nonAlphabets
    filterOutNonAlphabets(DATA_FILE_LONGWORDS, nonAlphabets, 'non_alphabets.txt')
